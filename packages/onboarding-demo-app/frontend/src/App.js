...
function App() {
  ...
  // Determine onboard/offboard status based on WLAN SSID
  useEffect(() => {
    const ssidMatch = wlanStatus.match(/SSID:\s*(\S+)/);
    const ssid = ssidMatch ? ssidMatch[1] : null;

    if (ssid === 'registrar-tls-ca') {
      setOnboardStatus("Status: Onboarded");
      setOffboardStatus("Status: Unknown");
    } else if (ssid === 'brski-open') {
      setOnboardStatus("Status: Open Network");
      setOffboardStatus("Status: Unknown");
    } else {
      ...
      <div className="action-grid">
        ...
        <div className="cyber-card onboard-card">
          ...
          <div className={`status-indicator ${
            onboardStatus.includes('Onboarded') ? 'status-onboarded' : 
            onboardStatus.includes('Open Network') ? 'status-partial' : ''
          }`}>
            {onboardStatus}
          </div>
          ...
          {onboardLog && <pre className="log-output">{onboardLog}</pre>}
        </div>
        ...
        <div className="cyber-card offboard-card">
          ...
          {offboardLog && <pre className="log-output">{offboardLog}</pre>}
        </div>
      </div>
